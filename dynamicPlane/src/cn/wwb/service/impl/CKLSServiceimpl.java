...
public class CKLSServiceimpl implements CKLSService {
	...
	public Map<String, Object> search(int currentPage, int lineSize, String FLID) throws Exception {
		...
		try {
			map.put("Hangbans", DAOFactory.getCKLSdaoinstance(this.dbc.getConn()).findBySplit(currentPage, lineSize,FLID));
			map.put("counts", DAOFactory.getCKLSdaoinstance(this.dbc.getConn()).getCount(FLID));
			return map;
		} catch (Exception e) {
			throw e;
		} finally {
